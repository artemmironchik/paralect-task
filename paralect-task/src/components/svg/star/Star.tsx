...
export default function Star({ vacancy, handleIconClick }: StarProps) {
  const { classes } = useStyles();
  const [isFavorite, setIsFavorite] = useState(vacancy.favorite);

  const handleClick = () => {
    handleIconClick(vacancy);
  ...
  return (
    ...
    >
      <svg
        className={isFavorite ? [classes.starIcon, classes.active].join(' ') : classes.starIcon}
        width="22"
        height="20"
        viewBox="0 0 22 20"
        fill="#FFFFFF"
